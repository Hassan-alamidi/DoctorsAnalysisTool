...
public interface TreatmentPlanDAO extends JpaRepository<TreatmentPlan, Integer> {
    ...
    @Query("SELECT t FROM TreatmentPlan t WHERE t.patient.ppsn = ?1 ORDER BY t.startDate DESC")
    List<TreatmentPlan> getAllTreatments(final String ppsn);

    @Query("SELECT t FROM TreatmentPlan t WHERE t.patient.ppsn = ?1 AND (t.endDate > CURRENT_DATE() OR t.endDate IS NULL)")
    List<TreatmentPlan> getAllOnGoingTreatments(final String ppsn);

    @Query("SELECT t FROM TreatmentPlan t WHERE t.patient.ppsn = ?1 AND t.endDate < CURRENT_DATE()")
